...
export const getCharacters = (character) => {
  ...
  const URL = `https://gateway.marvel.com:443/v1/public/characters?ts=${TIME_STAMP}&nameStartsWith=${character}&limit=50&apikey=${PUBLIC_KEY}&hash=${HASH.hex()}`;
  return (dispatch) => {
    fetch(URL)
...
export const getCharacterComics = (id) => {
  ...
  const HASH = md5.create();
  HASH.update(TIME_STAMP + PRIVATE_KEY + PUBLIC_KEY);
  const URL = `https://gateway.marvel.com:443/v1/public/characters/${id}/comics?ts=${TIME_STAMP}&limit=50&apikey=${PUBLIC_KEY}&hash=${HASH.hex()}`;
  return (dispatch) => {
    fetch(URL)
      .then((response) => response.json())
      .then((data) =>
...
export const getComics = () => {
  ...
  const HASH = md5.create();
  HASH.update(TIME_STAMP + PRIVATE_KEY + PUBLIC_KEY);
  const URL = `https://gateway.marvel.com:443/v1/public/comics?format=comic&noVariants=true&ts=${TIME_STAMP}&limit=50&apikey=${PUBLIC_KEY}&hash=${HASH.hex()}
  `;
  return (dispatch) => {
    fetch(URL)
      .then((response) => response.json())
